	...
	if(sizeof($delayedgamKlData) > 0) { ?>
		<table class="reportTable">
			...
			<?php
				...
				foreach($delayedgamKlData as $key => $val) {
					echo
						"<tr>"
							. "<td>#{$val['nr']}, {$val['sutarties_data']}</td>"
							. "<td>{$val['vardas']} {$val['pavardė']}</td>"
		...
		<a href="index.php?module=sutartys&action=report_delayed_gamKl" title="Nauja ataskaita" style="margin-bottom: 15px" class="button large float-right">nauja ataskaita</a>
<?php   
	} else { 
?>
